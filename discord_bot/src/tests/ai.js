#!/usr/bin/env node
/**
 * Έλεγχοι ασφαλείας και ορίων για το AI.
 *
 *   node src/tests/run.js ai
 *
 * ΟΛΑ τρέχουν χωρίς δίκτυο, με εικονικό `fetch`. Ένα τεστ ασφαλείας που
 * χρειάζεται κλειδί δεν τρέχει ποτέ στη μηχανή που το χρειάζεται.
 */
const fs = require('fs');
...
async function main() {
  ...
  // ΤΟ ΠΡΩΤΟ ΠΟΥ ΓΡΑΦΤΗΚΕ. Το επίπεδο 3 δεν φιλτράρεται από την έξοδο — απουσιάζει
  // από την είσοδο. Το μοντέλο δεν έχει token για να το εκφράσει.
  {
    ...
    // Κάθε επιτρεπόμενη ενέργεια χρειάζεται εκτελεστή, αλλιώς το μοντέλο τη
    // ζητάει και δεν γίνεται τίποτα — φαίνεται σαν αγνοημένο αίτημα.
    ...
    // Και το αντίστροφο: εκτελεστής που δεν είναι στο enum είναι νεκρός κώδικας
    // ή, χειρότερα, δρόμος που κάποιος θα κάνει προσβάσιμο κατά λάθος.
  ...
  {
    // Κανένα από αυτά δεν πρέπει να καταλήξει σε ενέργεια. Ελέγχεται στον
    // εφεδρικό router, που είναι η διαδρομή ΧΩΡΙΣ μοντέλο — δηλαδή χωρίς καμία
    // κρίση, μόνο μοτίβα. Αν περνούσε κάτι εδώ, θα περνούσε πάντα.
    const HOSTILE = [
    ...
    // Ό,τι όντως δρομολογήθηκε πρέπει να είναι στη λίστα επιτρεπτών.
  ...
  section('χωρίς κλειδί, το fetch δεν καλείται ΠΟΤΕ');
  {
    ...
    // Η εντολή δεν πρέπει καν να φορτώνεται χωρίς κλειδί — κριτήριο αποδοχής:
    // μηδενική αλλαγή συμπεριφοράς σε εγκατάσταση χωρίς κλειδί.
  ...
  section('με εικονικό κλειδί: το σχήμα φεύγει, τα μυστικά όχι');
  {
    ...
    // Ο,τι στέλνεται πρέπει να είναι μόνο όσα έγραψε ο χρήστης, κομμένα.
  ...
  {
    // Πάροχος που αγνοεί εντελώς το responseSchema. Δεν είναι υποθετικό: ένα
    // μοντέλο σε φόρτο επιστρέφει ό,τι θέλει, και η δομημένη έξοδος είναι
    // αίτημα, όχι εγγύηση.
    const rogue = async () => ({
      ok: true,
      json: async () => ({
  ...
  {
    // Το μοντέλο γράφει την απάντησή του ΠΡΙΝ εκτελεστεί οτιδήποτε. Ενώνοντας
    // τα δύο έβγαινε «Βάζω αμέσως το Mad Clip!» και από κάτω «Αυτό δουλεύει
    // μόνο μέσα σε server» — υπόσχεση και διάψευση στο ίδιο μήνυμα.
  ...
  {
    // Ένα «βάλε Mad Clip» από DM έβγαζε τέσσερα μηνύματα: την απάντηση της
    // εντολής, το embed «Now Playing», το «δοκιμάζω SoundCloud…» και το
    // αποτέλεσμά του.
    const playedWith = [];
    const fakeClient = {
      player: {
    ...
    // Μέσα σε server το embed πρέπει να συνεχίσει να ανακοινώνεται κανονικά —
    // εκεί το κανάλι είναι κοινό και η ανακοίνωση έχει νόημα για όλους.
  ...
  section('εύρεση του guild από το κανάλι φωνής (για DM)');
  {
    ...
    // Φρέσκος client σε κάθε κλήση: το `cache.values()` είναι iterator και
    // καταναλώνεται, οπότε ένας κοινός θα έδινε άδεια λίστα στη δεύτερη χρήση
    // και ψεύτικα «δεν βρέθηκε».
    const freshClient = () => ({
      guilds: {
        cache: {
          values: () => [
    ...
    // Κάποιος που δεν είναι πουθενά σε φωνή δεν πρέπει να «βρεθεί» κάπου.
    ...
    // Το context πρέπει να αποκτήσει guild, χωρίς να χάσει ό,τι είχε.
    ...
    // Ένα context που ΗΔΗ έχει guild δεν πρέπει να ξαναψάχνεται.
  ...
  section('μνήμη συνομιλίας');
  {
    ...
    // Χωρίς έξωση, ένας πολυσύχναστος server μεγαλώνει το Map επ' άπειρον —
    // διαρροή που φαίνεται μόνο μετά από εβδομάδες uptime.
  ...
  section('ημερήσιο budget (επιβιώνει σε restart)');
  {
    ...
    // Ο λόγος που ζει στη βάση: ένας μετρητής στη μνήμη μηδενιζόταν σε κάθε
    // deploy, ενώ η πραγματική ποσόστωση της Google δεν μηδενίζεται μαζί του.
