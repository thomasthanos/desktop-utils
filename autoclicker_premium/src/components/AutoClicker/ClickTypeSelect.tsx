...
interface ClickTypeSelectProps {
  value: ClickType;
...
const ClickTypeSelect = ({ value, onChange }: ClickTypeSelectProps) => {
  ...
  const selectedLabel = options.find(o => o.value === value)?.label || 'Single';

  return (
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-2">
        <MousePointerClick className="w-4 h-4 text-muted-foreground" />
        <span className="text-sm text-foreground">Click Type</span>
      </div>
...
export default ClickTypeSelect;
